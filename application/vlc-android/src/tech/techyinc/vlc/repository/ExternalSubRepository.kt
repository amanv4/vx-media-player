...
import tech.techyinc.vlc.database.ExternalSubDao
...
import tech.techyinc.tools.livedata.LiveDataMap
import java.io.File
...
class ExternalSubRepository(private val externalSubDao: ExternalSubDao, private val coroutineContextProvider: CoroutineContextProvider = CoroutineContextProvider()) {
    ...
    fun saveDownloadedSubtitle(idSubtitle: String, subtitlePath: String, mediaPath: String, language: String, movieReleaseName: String): Job {
        return GlobalScope.launch(coroutineContextProvider.IO) { externalSubDao.insert(tech.techyinc.vlc.mediadb.models.ExternalSub(idSubtitle, subtitlePath, mediaPath, language, movieReleaseName)) }
    }

    fun getDownloadedSubtitles(mediaUri: Uri): LiveData<List<tech.techyinc.vlc.mediadb.models.ExternalSub>> {
        val externalSubs = externalSubDao.get(mediaUri.path!!)
        return Transformations.map(externalSubs) { list ->
            val existExternalSubs: MutableList<tech.techyinc.vlc.mediadb.models.ExternalSub> = mutableListOf()
            list.forEach {
