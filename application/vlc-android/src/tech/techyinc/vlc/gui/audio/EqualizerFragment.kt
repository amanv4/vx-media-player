...
import tech.techyinc.resources.VLCOptions
import tech.techyinc.tools.Settings
import tech.techyinc.tools.isStarted
import tech.techyinc.vlc.BuildConfig
import tech.techyinc.vlc.PlaybackService
import tech.techyinc.vlc.R
import tech.techyinc.vlc.databinding.EqualizerBinding
import tech.techyinc.vlc.gui.dialogs.VLCBottomSheetDialogFragment
import tech.techyinc.vlc.gui.helpers.UiTools
import tech.techyinc.vlc.gui.view.EqualizerBar
import tech.techyinc.vlc.interfaces.OnEqualizerBarChangeListener
...
class EqualizerFragment : VLCBottomSheetDialogFragment() {
    ...
    private val newPresetName = AppContextProvider.appResources.getString(tech.techyinc.vlc.R.string.equalizer_new_preset_name)
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding = DataBindingUtil.inflate(inflater, tech.techyinc.vlc.R.layout.equalizer, container, false)
        binding.state = state
        return binding.root
    }
    ...
    private fun fillViews() = lifecycleScope.launch(start = CoroutineStart.UNDISPATCHED) {
        ...
        eqBandsViews[0].nextFocusLeftId = tech.techyinc.vlc.R.id.equalizer_preamp
        eqBandsViews[eqBandsViews.size - 1].nextFocusRightId = tech.techyinc.vlc.R.id.snapBands
    ...
    private fun createSaveCustomSetDialog(positionToSave: Int, displayedByUser: Boolean, onPause: Boolean) {
        ...
        val klNormal = resources.getDimension(tech.techyinc.vlc.R.dimen.kl_normal).toInt()
    ...
    private fun createDeleteCustomSetSnacker() {
        ...
        if (getEqualizerType(oldPos) == TYPE_CUSTOM) {
            ...
            val message = getString(tech.techyinc.vlc.R.string.custom_set_deleted_message, oldName)
    ...
    private fun revertCustomSetChanges() {
        ...
        val message = if (getEqualizerType(pos) == TYPE_CUSTOM)
            getString(tech.techyinc.vlc.R.string.custom_set_restored)
        else
            getString(tech.techyinc.vlc.R.string.unsaved_set_deleted_message)
        UiTools.snackerWithCancel(requireActivity(), message, null, cancelAction)
    }
