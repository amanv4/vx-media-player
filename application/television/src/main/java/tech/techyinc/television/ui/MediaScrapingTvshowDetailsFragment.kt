...
import tech.techyinc.moviepedia.database.models.*
import tech.techyinc.moviepedia.viewmodel.MediaMetadataFull
import tech.techyinc.moviepedia.viewmodel.MediaMetadataModel
import tech.techyinc.resources.util.getFromMl
import tech.techyinc.tools.HttpImageLoader
...
import tech.techyinc.vlc.util.getScreenWidth
...
class MediaScrapingTvshowDetailsFragment : DetailsSupportFragment(), CoroutineScope by MainScope(), OnItemViewClickedListener {
    ...
    private lateinit var detailsDescriptionPresenter: tech.techyinc.television.ui.TvShowDescriptionPresenter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        detailsDescriptionPresenter = tech.techyinc.television.ui.TvShowDescriptionPresenter()
        arrayObjectAdapterPosters = ArrayObjectAdapter(tech.techyinc.television.ui.MediaImageCardPresenter(requireActivity(), MediaImageType.POSTER))

        val extras = requireActivity().intent.extras ?: savedInstanceState ?: return
        showId = extras.getString(tech.techyinc.television.ui.TV_SHOW_ID) ?: return
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.putString(tech.techyinc.television.ui.TV_SHOW_ID, showId)
        ...
        when (item) {
            ...
            is MediaMetadataWithImages -> {
                launch {
                    ...
                    media?.let {
                        tech.techyinc.television.ui.TvUtil.showMediaDetail(requireActivity(), it)
                    }
                }
            }
        }
    ...
    private fun updateMetadata(tvshow: MediaMetadataFull?) {
        ...
        tvshow?.let { tvshow ->
            ...
            tvshow.seasons?.let {
                it.forEach {
                    val arrayObjectAdapterSeason = ArrayObjectAdapter(tech.techyinc.television.ui.MetadataCardPresenter(requireActivity()))
                    arrayObjectAdapterSeason.setItems(it.episodes, tech.techyinc.television.ui.TvUtil.metadataDiffCallback)
            ...
            tvshow.metadata?.let { metadata ->
                ...
                if (metadata.images.any { it.imageType == MediaImageType.BACKDROP }) {
                    val arrayObjectAdapterBackdrops = ArrayObjectAdapter(tech.techyinc.television.ui.MediaImageCardPresenter(requireActivity(), MediaImageType.BACKDROP))
            ...
            rowsAdapter.setItems(items, object : DiffCallback<Row>() {
                ...
                override fun areContentsTheSame(oldItem: Row, newItem: Row): Boolean {
                    if (oldItem is DetailsOverviewRow && newItem is DetailsOverviewRow) {
                        return oldItem.item as tech.techyinc.television.ui.MediaItemDetails == newItem.item as tech.techyinc.television.ui.MediaItemDetails
                    }
                    return true
                }
            })
    ...
    private fun buildDetails(tvShow: MediaMetadataFull) {
        ...
        val videoPresenter = tech.techyinc.television.ui.VideoDetailsPresenter(requireActivity(), requireActivity().getScreenWidth())
        ...
        rowPresenter.onActionClickedListener = OnActionClickedListener { action ->
            when (action.id.toInt()) {
                ...
                ID_START_OVER -> {
                    tech.techyinc.television.ui.TvUtil.playMedia(activity, viewModel.provider.getAllMedias(viewModel.updateLiveData.value?.seasons))
                }
            }
        }
        selector.addClassPresenter(DetailsOverviewRow::class.java, rowPresenter)
        selector.addClassPresenter(tech.techyinc.television.ui.VideoDetailsOverviewRow::class.java, videoPresenter)
