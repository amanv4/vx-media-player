...
import tech.techyinc.moviepedia.database.models.MediaMetadataWithImages
...
import tech.techyinc.television.ui.NowPlayingDelegate
import tech.techyinc.television.ui.audioplayer.AudioPlayerActivity
import tech.techyinc.television.ui.browser.TVActivity
...
import tech.techyinc.vlc.util.isSchemeFile
import tech.techyinc.vlc.util.scanAllowed
                ...
                item.metadata.mlId?.let {
                    viewModelScope.launch {
                        context.getFromMl {
                            ...
                            val intent = Intent(activity, tech.techyinc.television.ui.DetailsActivity::class.java)
                            // pass the item information
                            intent.putExtra("media", it)
                            intent.putExtra("item", tech.techyinc.television.ui.MediaItemDetails(it.title, it.artist, it.album, it.location, it.artworkURL))
                            activity.startActivity(intent)
                        }
                    }
                }
            }
            is MediaLibraryItem -> tech.techyinc.television.ui.TvUtil.openAudioCategory(activity, item)
        }
    }
